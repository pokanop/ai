...
//
// It also verifies the generated catalog has not drifted:
//   - skills/index.json exists and exactly matches what sync-docs.mjs would
//     emit from the current skills/ directory (run `bun run sync` to refresh it)
//
...
function main() {
  ...
  // Third pass: the generated catalog must match the skills on disk.
  validateIndex();
...
// ─── Catalog drift check ───────────────────────────────────────────────────
// skills/index.json is generated by sync-docs.mjs from the same SKILL.md
// frontmatter validated above. Recompute the expected catalog and compare it to
// the committed file byte-for-byte. A mismatch means someone changed a skill
// without regenerating the index — fail loudly so the catalog can't drift.
function validateIndex() {
  let expected;
  try {
    expected = serializeIndex(buildSkillsIndex());
  } catch (e) {
    error(INDEX_REL, `could not build the expected catalog: ${String(e?.message || e)}`);
    return;
  }

  if (!existsSync(INDEX_PATH)) {
    error(INDEX_REL, 'is missing — run `bun run sync` to generate the skill catalog');
    return;
  }

  if (readFileSync(INDEX_PATH, 'utf-8') !== expected) {
    error(
      INDEX_REL,
      'is out of date with the skills/ directory — run `bun run sync` to regenerate it (the catalog must not drift)',
    );
  }
}
